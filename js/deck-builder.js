...
/**
 * Shared inner helper: builds the common head/styles/live pipeline.
 * @returns {{esc:string, sourcesJson:string, directionCss:string}}
 */
function buildDeckPayload(markdownContent) {
...
export function deckDocument(
  ...
  const { esc, sourcesJson } = buildDeckPayload(markdownContent);
  return `<!doctype html>
...
/**
 * Vector PDF / print variant of deckDocument.
 * Uses Reveal's `view:"print"` (reveal-print) which creates one .pdf-page
 * per slide via PrintView — vector, selectable text, exact theme/fonts.
 * The returned HTML auto-triggers print after `pdf-ready`.
 */
export function deckPrintDocument(markdownContent, themeCss, dir = "ltr", lang = "en", title = "slides") {
  const { esc, sourcesJson } = buildDeckPayload(markdownContent);
  const safeTitle = (title || "slides").replace(/</g, "&lt;").replace(/<\/title/gi, "<\\/title");
  const dirAttr = dir === "rtl" ? "rtl" : "ltr";
  return `<!doctype html>
  <html class="reveal-print print-pdf" dir="${dirAttr}" lang="${lang}">
  <head>
  <meta charset="utf-8">
...
.lk-quiz-explain{ margin-top:12px; font-size:14px; color:#b9b8ce; font-style:italic; }
  ...
  <style>
    *{ -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
    @page{ margin:0; }
    html.reveal-print body, html.print-pdf body{ background: var(--r-background-color, #040810) !important; background-color: var(--r-background-color, #040810) !important; }
    #print-status{ position:fixed; top:12px; left:12px; z-index:9999; background:#0b1120; color:#eef1fa; border:1px solid #2a3145; padding:8px 12px; border-radius:6px; font:13px/1 Inter,sans-serif; }
    @media print{ #print-status{ display:none !important; } }
  </style>
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:wght@600&family=Inter:wght@400;600&family=JetBrains+Mono&family=Noto+Sans+Arabic:wght@400;600&display=swap" rel="stylesheet">
  </head>
  <body><div id="print-status">Preparing PDF… will open print dialog automatically. Choose "Save as PDF" and enable "Background graphics".</div>
  <div class="reveal">
    <div class="slides">
      <section data-markdown data-separator="^\\n---\\n$" data-separator-vertical="^\\n--\\n$" data-separator-notes="^Note:">
        <textarea data-template>${esc}</textarea>
  ...
  window.__lkCtx = window.__lkCtx || {};
  const ctxKey = 'b' + idx + '_' + Date.now() + '_' + Math.random().toString(36).slice(2);
  window.__lkCtx[ctxKey] = { container: stage, LiveKit: window.LiveKit, d3: window.d3 };
  const moduleSrc = ['const __ctx = window.__lkCtx["' + ctxKey + '"];','const container = __ctx.container;','const LiveKit = __ctx.LiveKit;','const d3 = __ctx.d3;',source].join(String.fromCharCode(10));
  const blob = new Blob([moduleSrc], { type: 'text/javascript' });
  const url = URL.createObjectURL(blob);
  import(/* webpackIgnore: true */ url).then(()=>{
      delete window.__lkCtx[ctxKey]; URL.revokeObjectURL(url);
      scheduleLayout(); requestAnimationFrame(scheduleLayout); setTimeout(scheduleLayout,60); setTimeout(scheduleLayout,250);
    }).catch((err)=>{
      stage.innerHTML = ''; const e=document.createElement('div'); e.className='live-error'; e.textContent=(err&&err.message)?err.message:String(err); stage.appendChild(e);
      delete window.__lkCtx[ctxKey]; URL.revokeObjectURL(url); scheduleLayout(); requestAnimationFrame(scheduleLayout); setTimeout(scheduleLayout,60);
    });
        }
        rerun.addEventListener('click', run); run();
      });
      scheduleLayout(); requestAnimationFrame(scheduleLayout); setTimeout(scheduleLayout,60);
    }

    // Reveal in print view: creates one .pdf-page per slide, exact styling preserved
    const deck = new Reveal({
      hash: false, controls:false, progress:false, center:true,
      view: 'print',
    ...
    function doPrint(){
      ...
      if(el) el.textContent='Ready — opening print dialog. Save as PDF to download.';
      setTimeout(function(){ try{ window.print(); }catch(e){} setTimeout(function(){ if(el) el.style.display='none'; },1000); },600);
    }
    deck.initialize().then(()=>{
      activateLiveBlocks();
      scheduleLayout(); requestAnimationFrame(scheduleLayout); setTimeout(scheduleLayout,80); setTimeout(scheduleLayout,350); setTimeout(scheduleLayout,800);
      try{
        const ro=new ResizeObserver(()=>scheduleLayout());
        document.querySelectorAll('.live-stage').forEach(el=>ro.observe(el));
        document.querySelectorAll('.reveal .slides section').forEach(el=>ro.observe(el));
      ...
      // reveal's print view disables normal slidechanged etc, but still fires pdf-ready
      // Hook both Reveal events and document events
      try{
        deck.on('pdf-ready', function(){ window.__pdfReadyFired=true; doPrint(); });
        deck.on('ready', function(){ scheduleLayout(); });
      }catch(e){}
      document.addEventListener('pdf-ready', function(){ window.__pdfReadyFired=true; doPrint(); });
      // Fallback if pdf-ready doesn't fire
      setTimeout(function(){ if(!window.__pdfReadyFired && document.querySelector('.pdf-page')) doPrint(); }, 2500);
      setTimeout(function(){ if(!window.__pdfReadyFired) doPrint(); }, 4000);
    });
    window.addEventListener('resize', ()=>{ try{ scheduleLayout(); }catch(e){} });
  <\/script>
  </body>
