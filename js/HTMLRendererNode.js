...
qInput.style.background = '#0b1120';
qInput.style.color = '#eef1fa';
qInput.style.border = '1px solid #2a3145';
...
optInput.style.background = '#0b1120';
optInput.style.color = '#eef1fa';
optInput.style.border = '1px solid #2a3145';
...
dynamicLabel.style.color = '#4992ff';
...
dynamicInput.style.background = '#0b1120';
dynamicInput.style.color = '#eef1fa';
dynamicInput.style.border = '1px solid #2a3145';
...
explanation.style.borderLeft = '4px solid #7cffb2';
explanation.style.background = '#0b1120';
explanation.style.color = '#fddd60';
...
options.forEach((option, index) => {
  ...
  button.addEventListener('click', () => {
    ...
    if (index === correctIndex) {
      ...
      button.style.background = '#7cffb222';
      button.style.borderColor = '#7cffb2';
      button.style.color = '#7cffb2';

    } else {
      // Incorrect Answer Styling
      button.style.background = '#ff6e7622';
      button.style.borderColor = '#ff6e76';
      button.style.color = '#ff6e76';

      // Highlight the actual correct answer for the user
      const correctBtn = choices.children[correctIndex];
      correctBtn.style.background = '#7cffb222';
      correctBtn.style.borderColor = '#7cffb2';
      correctBtn.style.color = '#7cffb2';
    ...
    return [
      {
        ...
        getAttrs: (element) => {
          let configGenerator =
            element.getAttribute("data-config-generator") || "";
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      const extractManagedKeys = (script) => {
        ...
        const stateMatches = script.matchAll(
          /state\s*(?:\??\.)\s*([a-zA-Z0-9_]+)/g,
        );
        for (const match of stateMatches) {
          keys.add(match[1]);
        }

        const stateBracketMatches = script.matchAll(
          /state\[['"]([a-zA-Z0-9_]+)['"]\]/g,
        );
        ...
        const updateMatches = script.matchAll(
          /updateState\s*\(\s*\{([^}]*)\}/g,
        );
      ...
      const updateTabStyles = () => {
        if (tabPreview.classList.contains("active")) {
          Object.assign(tabPreview.style, {
            background: "#0b1120",
            borderColor: "#2a3145",
            color: "#4992ff",
          });
          Object.assign(tabCode.style, {
            background: "transparent",
      ...
      renderViewPaneLabel.textContent =
        "Render View Output (isolated iframe, rendered against state):";
      renderViewPaneLabel.className = "editor-label";
      renderViewPaneLabel.style.color = "#4992ff";
      ...
      configGenLabel.textContent =
        "Config Generator (JS) - Use `container`, `state`, and `updateState()`:";
      configGenLabel.className = "editor-label";
      configGenLabel.style.color = "#4992ff";
      ...
      renderViewLabel.textContent =
        "Render View (Mustache Template) - Automatically rendered against state:";
      renderViewLabel.className = "editor-label";
      renderViewLabel.style.color = "#4992ff";
      ...
      fullscreenBtn.style.background = "#0b1120";
      fullscreenBtn.style.color = "#eef1fa";
      fullscreenBtn.style.border = "1px solid #2a3145";
      ...
      const exitFullscreen = () => {
        ...
        ["flex", "maxHeight", "overflow"].forEach((prop) => {
        ...
        [
          ...
          renderViewIframe.style[prop] = "";
        });

        // Reset codeContainer styles
        ["flex", "display", "flexDirection", "minHeight", "overflow"].forEach(
          (prop) => {
      ...
      const renderPreviewConfig = (n) => {
          ...
          if (isStructural) {
            ...
            for (const key in updates) {
              ...
              if (
                ...
                Array.isArray(latestVal) &&
                nextVal.length > baseVal.length
              ) {
                let isExtension = true;
                for (let i = 0; i < baseVal.length; i++) {
                  if (
                    JSON.stringify(baseVal[i]) !== JSON.stringify(nextVal[i])
                  ) {
                    isExtension = false;
                    break;
                  }
        ...
        try {
          const configFunc = new Function(
            ...
            "state",
            "updateState",
            n.attrs.configGenerator,
          );
          configFunc(wrapper, currentState, updateState);
        } catch (e) {
          wrapper.innerHTML = `<pre style="color: #ff6e76; background: #0b1120; padding: 10px; border-radius: 4px; white-space: pre-wrap; border: 1px solid #2a3145;">Config Generator Error: ${e.message}</pre>`;
        }
      };
      ...
      const renderRenderViewPreview = (n) => {
        ...
        try {
          ...
          code =
            ...
            JSON.stringify(
              "Render View Error: " + (e && e.message ? e.message : String(e)),
            ) +
            ";";
        }
        ...
        renderViewIframe.srcdoc =
          ...
          "html,body{margin:0;padding:20px;background:#040810;font-family:system-ui,-apple-system,sans-serif;color:#eef1fa;}" +
          "</style></head><body>" +
          '<div id="rv-root"></div>' +
          ...
          '  pre.style.cssText = "color:#ff6e76;background:#0b1120;padding:10px;border-radius:4px;white-space:pre-wrap;border:1px solid #2a3145;";' +
      ...
      return {
        ...
        update: (updatedNode) => {
          ...
          const configGenChanged =
            updatedNode.attrs.configGenerator !==
            currentNode.attrs.configGenerator;
          ...
          const stateChanged =
            ...
            JSON.stringify(currentNode.attrs.state);
          ...
          const isPreviewFocused =
            activeEl && previewContainer.contains(activeEl);
          const isTextEditingFocused =
            isPreviewFocused &&
          ...
          if (codeContainer.style.display !== "none" || isFullscreen) {
            if (
              updatedNode.attrs.configGenerator !== configGenEditor.getValue()
            ) {
